...
const ICON_MAP = {
  // Transport
  'bus_icon': '🚌', 'car_icon': '🚗', 'bike_icon': '🚲', 'scooter_icon': '🛴',
  'metro_icon': '🚇', 'train_icon': '🚆', 'auto_icon': '🛺', 'walk_icon': '🚶',
  'cycling_icon': '🚴', 'public_transport': '🚌', 'green_commuter': '🚲',

  // Food
  'plant_icon': '🌱', 'meat_icon': '🍖', 'vegetarian_icon': '🥗', 'vegan_icon': '🌿',
  'food_icon': '🍽️', 'farm_icon': '🌾', 'local_icon': '🏪', 'seasonal_icon': '🍂',
  'meatless_mondays': '🥗', 'plant_based_month': '🌱', 'eat_local_seasonal': '🌾',

  // Energy
  'plug_icon': '🔌', 'energy_icon': '⚡', 'light_icon': '💡', 'solar_icon': '☀️',
  'unplug_vampire': '🔌', 'cold_water_wash': '👕',

  // Waste
  'recycle_icon': '♻️', 'trash_icon': '🗑️', 'plastic_icon': '🥤', 'waste_icon': '🚯',
  'zero_plastic': '🥤', 'zero_waste_lunch': '🍱',

  // Water
  'water_icon': '💧', 'shower_icon': '🚿', 'drop_icon': '💦',

  // Shopping
  'shop_icon': '🛍️', 'bag_icon': '🛒', 'cart_icon': '🛒', 'sustainable_shopper': '🛍️',

  // General / Fallback
  'leaf': '🍃', 'tree': '🌳', 'earth': '🌍', 'star': '⭐', 'trophy': '🏆',
  'target': '🎯', 'flame': '🔥', 'check': '✅', 'alert': '⚠️',
}

function getChallengeIcon(challenge) {
  // Priority 1: If it's already a real emoji (length 1-2 and not ASCII), use it
  if (challenge.icon && /^[^-]{1,2}$/.test(challenge.icon)) {
    return challenge.icon
  }
  // Priority 2: Map from icon field
  if (challenge.icon && ICON_MAP[challenge.icon]) {
    return ICON_MAP[challenge.icon]
  }
  // Priority 3: Map from challenge ID
  if (challenge.id && ICON_MAP[challenge.id]) {
    return ICON_MAP[challenge.id]
  }
  // Priority 4: Category-based fallback
  const categoryIcons = {
    'Transport': '🚌', 'Energy': '⚡', 'Food': '🍽️', 
    'Waste': '♻️', 'Water': '💧', 'Shopping': '🛍️'
  }
  return categoryIcons[challenge.category] || '🌱'
}
...
export default function Challenges() {
  ...
  const fetchChallenges = async () => {
    try {
      ...
      setChallenges([
        ...
        { id: 'public_transport_week', title: 'Public Transport Week', description: 'Challenge yourself to use public transport, cycle, or walk for your commutes and errands for an entire week', category: 'Transport', difficulty: 'Medium', duration: '7 days', ecoPoints: 150, co2Reduction: '1.5', participants: 1234, icon: '🚌' },
        { id: 'plant_based_month', title: 'Plant-Based Month', description: 'Commit to eating no meat or poultry every Monday for a month. Explore delicious plant-based recipes!', category: 'Food', difficulty: 'Easy', duration: '30 days', ecoPoints: 100, co2Reduction: '2.5', participants: 2567, icon: '🌱' },
        { id: 'unplug_vampire', title: 'Unplug Vampire Devices', description: 'Reduce standby power consumption by unplugging electronics when not in use or by utilizing smart power strips', category: 'Energy', difficulty: 'Easy', duration: 'Ongoing', ecoPoints: 80, co2Reduction: '1.2', participants: 3456, icon: '🔌' },
        { id: 'zero_waste_lunch', title: 'Zero-Waste Lunch Challenge', description: 'Prepare and pack your lunch without any single-use plastics, excessive packaging, or disposables for a full week', category: 'Waste', difficulty: 'Medium', duration: '7 days', ecoPoints: 120, co2Reduction: '1.8', participants: 1876, icon: '🍱' },
        { id: 'cold_water_wash', title: 'Cold Water Laundry', description: 'Wash all your laundry using cold water settings for two weeks. Most energy used for washing goes into heating water!', category: 'Energy', difficulty: 'Easy', duration: '14 days', ecoPoints: 90, co2Reduction: '2.1', participants: 2105, icon: '👕' },
        { id: 'eat_local_seasonal', title: 'Eat Local & Seasonal', description: 'For one month, prioritize purchasing local and seasonal produce and ingredients to reduce food miles', category: 'Food', difficulty: 'Medium', duration: '30 days', ecoPoints: 180, co2Reduction: '3.0', participants: 987, icon: '🌾' }
      ])
    }
  }
  ...
  const startChallenge = async (challenge) => {
    ...
    try {
      const res = await api.post('/api/challenges/start', {
  ...
  const checkIn = async (challengeId) => {
    ...
    try {
      const res = await api.post('/api/challenges/checkin', { challengeId })
      if (res.data?.success) {
              ...
              {filteredChallenges.map((challenge, index) => (
                <motion.div key={challenge.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <span className="text-3xl">{getChallengeIcon(challenge)}</span>
                      <div>
