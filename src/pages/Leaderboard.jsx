...
const Avatar = ({ avatar, size = 'text-2xl', imgClass = 'w-8 h-8' }) => {
  ...
  if (isImageUrl) {
    return <img src={avatar} alt="" className={`${imgClass} rounded-full object-cover`} />;
  }
                  ...
                  {[1, 0, 2].map((actualIndex, i) => {
                    ...
                    return (
                      <motion.div key={podiumUser.rank || actualIndex} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.2 }} className="flex flex-col items-center">
                        <div className="mb-2"><Avatar avatar={podiumUser.avatar} size="text-4xl" imgClass="w-12 h-12" /></div>
